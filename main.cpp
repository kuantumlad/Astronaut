...
//const char* poly_input
  ...
  std::string save_path = "/home/brandon/Documents/projects/Astronaut/pics/" + title;
 ...
 /* INIT POLYGONS */
  ...
  for(int s = 0; s < 16 ; s++ ){
    ...
    my_polys.push_back(  PolygonScreen(true_window_size, image_section[s], npolys_to_make, color_choice) );
    my_polys[s].setWigglerLimits(*my_poly_windows[s]);
    my_polys[s].setScreenNumber(s);
    my_polys[s].setSavePath(save_path);
  ...
  int poly_screen = 1;
  while( screen >= 0 ){    
    //std::cout << " >> IN SCREEN WHILE LOOP " << std::endl;
    for( int s = 0; s < 16; s++ ){
      //std::cout << " >> RUN WINDOW SECTION " << s << std::endl;
      //std::cout << " >> ADDRESS OF MY WINDOW " << (my_windows[s]) << std::endl;
      //if( s == 14 ){
